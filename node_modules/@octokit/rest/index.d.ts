...
declare namespace Octokit {
  ...
  export type ProjectsUpdateParams = {
    ...
    state?: "open" | "closed";
    /**
     * The permission level that determines whether all members of the project's organization can see and/or make changes to the project. If an organization member belongs to a team with a higher level of access or is a collaborator with a higher level of access, their permission level is not lowered by `organization_permission`. For information on changing access for a team or collaborator, see [Add or update team project](https://developer.github.com/v3/teams/#add-or-update-team-project) or [Add user as a collaborator](https://developer.github.com/v3/projects/collaborators/#add-user-as-a-collaborator).  ,*   ,* **Note:** Updating a project's `organization_permission` requires `admin` access to the project. Setting this permission is only available for organization projects.  ,*   ,* Can be one of:  ,* \* `read` - Organization members can read, but not write to or administer this project.  ,* \* `write` - Organization members can read and write, but not administer this project.  ,* \* `admin` - Organization members can read, write and administer this project.  ,* \* `none` - Organization members can only see this project if it is public.
     */
    organization_permission?: string;
    /**
     * Sets the visibility of the project within the organization. **Note:** Updating a project's visibility requires `admin` access to the project. Setting visibility is only available for organization projects.  ,*   ,* Can be one of:  ,* \* `false` - Anyone can see the project.  ,* \* `true` - Organization members with the appropriate `organization_permission` can see the project.
     */
    private?: boolean;
    /**
     * Results per page (max 100)
  ...
  export type ReposListBranchesParams = {
    ...
    repo: string;
    /**
     * Setting to `true` returns only protected branches and each branch in the response includes the `protected` and `protection` keys. When you omit this parameter, all branches are returned and protected branches do not contain the `protected` and `protection` keys.
     */
    protected?: boolean;
    /**
     * Results per page (max 100)
